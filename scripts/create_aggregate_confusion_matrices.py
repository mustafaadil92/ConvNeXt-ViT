...
FIGURES_ROOT = Path("outputs/aggregate_figures")
...
TASK_CONFIG = {
    ...
    "binary": {
        "pred_root": PREDICTIONS_ROOT / "binary",
...
def _normalize_binary_label(label: str) -> str:
    ...
    if key in BENIGN_SUBTYPES:
        return "benign"
    if key in MALIGNANT_SUBTYPES:
        return "malignant"
    raise ValueError(f"Unknown binary label mapping for '{label}'")


def _remap_matrix(labels: list[str], matrix: np.ndarray, task_name: str, target_labels: list[str]) -> np.ndarray:
    remapped = np.zeros((len(target_labels), len(target_labels)), dtype=float)

    if task_name == "binary":
        label_to_target = {_normalize_binary_label(label): idx for idx, label in enumerate(target_labels)}
        for src_i, row_label in enumerate(labels):
            dst_i = label_to_target[_normalize_binary_label(row_label)]
            for src_j, col_label in enumerate(labels):
                dst_j = label_to_target[_normalize_binary_label(col_label)]
                remapped[dst_i, dst_j] += matrix[src_i, src_j]
        return remapped
...
def _load_model_fold_matrices(model_dir: Path, task_name: str, target_labels: list[str]) -> np.ndarray:
    ...
    for path in matrix_paths:
        ...
        matrices.append(_remap_matrix(labels, matrix, task_name, target_labels))
...
def generate_confusion_matrices() -> list[Path]:
    ...
    for task_name, task_config in TASK_CONFIG.items():
        ...
        for model_name in MODEL_ORDER:
            ...
            fold_matrices = _load_model_fold_matrices(model_dir, task_name, target_labels)
...
def main() -> None:
    ...
    print("Generated aggregate confusion matrices:")
