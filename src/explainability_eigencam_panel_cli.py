...
from src.explainability import (
    extract_convnext_eigencam,
    extract_hybrid_attention_map,
    extract_hybrid_eigencam,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate ConvNeXt / ViT / hybrid explanation panels with Eigen-CAM")
    ...
    parser.add_argument("--output", type=str, default="outputs/figures/explanations/explanation_eigencam_panel.png")
    ...
    parser.add_argument("--output-dir", type=str, default="outputs/figures/explanations/batch_eigencam")
...
def _build_panel(
    ...
    convnext_cam, convnext_pred = extract_convnext_eigencam(convnext_model, convnext_tensor)
    vit_attn, vit_pred = extract_vit_attention_map(vit_model, vit_tensor)
    hybrid_cam, hybrid_pred = extract_hybrid_eigencam(hybrid_model, hybrid_tensor)
    ...
    panel_items = [
        ...
        ("ConvNeXt Eigen-CAM", overlay_heatmap(panel_image, convnext_map)),
        ("ViT Attention", overlay_heatmap(panel_image, vit_map)),
        ("Hybrid Eigen-CAM", overlay_heatmap(panel_image, hybrid_cam_map)),
    ...
    footer_lines = [
        f"sample={image_path} | label={sample_label}",
...
def _run_single(args, bundles, device: torch.device):
    ...
    print(f"Saved Eigen-CAM explanation panel: {out_path}")
...
def _run_batch(args, bundles, device: torch.device):
    ...
    print(f"Saved batch Eigen-CAM explanation panels under: {output_dir}")
    if insufficient:
        print("Quota warning:")
