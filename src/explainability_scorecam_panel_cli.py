...
from src.explainability import (
    extract_convnext_scorecam,
    extract_hybrid_attention_map,
    extract_hybrid_scorecam,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate ConvNeXt / ViT / hybrid explanation panels with Score-CAM")
    ...
    parser.add_argument("--scorecam-channels", type=int, default=32)
    parser.add_argument("--output", type=str, default="outputs/figures/explanations/explanation_scorecam_panel.png")
    ...
    parser.add_argument("--output-dir", type=str, default="outputs/figures/explanations/batch_scorecam")
...
def _build_panel(
    ...
    convnext_cam, convnext_pred = extract_convnext_scorecam(
    ...
    hybrid_cam, hybrid_pred = extract_hybrid_scorecam(
        model=hybrid_model,
    ...
    panel_items = [
        ...
        ("ConvNeXt Score-CAM", overlay_heatmap(panel_image, convnext_map)),
        ("ViT Attention", overlay_heatmap(panel_image, vit_map)),
        ("Hybrid Score-CAM", overlay_heatmap(panel_image, hybrid_cam_map)),
    ...
    footer_lines = [
        ...
        f"predictions: convnext={convnext_pred}, vit={vit_pred}, hybrid={hybrid_pred} | alpha={alpha:.2f} | scorecam_channels={scorecam_channels}",
    ]
...
def _run_single(args, bundles, device: torch.device):
    ...
    out_path = _build_panel(
        ...
        alpha=args.alpha,
        scorecam_channels=args.scorecam_channels,
    ...
    print(f"Saved Score-CAM explanation panel: {out_path}")
...
def _run_batch(args, bundles, device: torch.device):
    ...
    print(f"Saved batch Score-CAM explanation panels under: {output_dir}")
    if insufficient:
        print("Quota warning:")
